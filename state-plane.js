import {geoMercator, geoAlbers, geoTransverseMercator, geoConicConformal} from 'd3-geo';
...
export default function(state, width, height) {
        width = width || 1000;
        height = height || 500;
        if (statePlaneDef[state]) {
            let def = statePlaneDef[state];
            let projName = prj[def.proj] || def.proj;
            let proj = projName();
